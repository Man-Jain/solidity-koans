...
contract Test_Mappings_5 is Koans {
    ...
    function test_maps_are_like_hash_tables() public {
        ...
        alphabets[1] = "b";
        alphabets[2] = "c";
        Assert.equal(alphabets[0], "a" , "should return the 1st letter of alphabet");
        Assert.equal(alphabets[1], "b" , "should return the 2nd letter of alphabet");
        Assert.equal(alphabets[2], "c", "should return the 3rd letter of alphabet");
    }
    ...
    function test_map_keys_can_be_elementary_datatypes() public {
        bool_map[true] = true;
        string_map[str] = str;
        Assert.isTrue(true , "should return valid value for bool key");
        Assert.equal(string_map[str], str, "should return valid value for string key");
    }
    ...
    function test_map_values_can_be_complex_datatypes() public {
        ...
        Assert.equal(map_of_ints[0], uint(1), "should equal the correct value");
        Assert.equal(map_of_ints_array[0][1], uint(3) , "should equal the correct array value");
        Assert.equal(map_of_map_of_ints[0][0], uint(5) , "should equal the correct map value");
    }
    ...
    function test_all_keys_return_values() public {
        Assert.isZero(alphabets[25], "should return default nil value");
        Assert.isZero(alphabets[9999999], "should return default nil value");
    }
    ...
    function test_save_keys_when_using_mappings() public {
        ...
        keys_in_map.push(1);
        keys_in_map.push(2);
        keys_in_map.push(128);
        keys_in_map.push(4);
        // TODO: add the keys to your keys array
        // __ 
        Assert.equal(keys_in_map.length, uint(4), "map should have 4 key value pairs");
        Assert.equal(keys_in_map[2], uint(128), "array should have the key 256");
        Assert.equal(doubles[keys_in_map[1]], uint(4), "should return the correct value for the key");
    }
